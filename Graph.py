...
class Graph:
    ...
    def getHamiltonian(self):
        ...
        # base case given in the assignment which is ..
        if self.totalV < 3:
            return None

        # Create a Hamiltonian object of type walk
        Hamiltonian = Walk(self.totalV+1)

        # set the Hamilitonian walk by assuming vertex 0 as the first vertex in the walk
        # If there is a Hamilitonian circuit, then the path can be started from any point of circuit
        Hamiltonian.vertices[0] = 0
        Hamiltonian.totalV = self.totalV+1
        # Call function tryVisiting to return True if a Hamilton circuit has been found
        # And update the Hamiltonian walk and false otherwise
        if (self.tryVisiting(0, 1, Hamiltonian) == False):
            return None
        else:
            return Hamiltonian
    ...
    def tryVisiting(self, vertex, totalvisited, Hamiltonian):
        ...
        previousVertex = Hamiltonian.vertices[totalvisited-1]
        assert(previousVertex == vertex)
        # base case: which is if all vertices are included in the walk
        if totalvisited == (self.totalV):
            # The last vertex has to be adjacent to the first vertex in order for it to be a cycle for Hamilitonian
            if self.edges[previousVertex][Hamiltonian.vertices[0]] == 1:
                Hamiltonian.vertices[self.totalV] = 0
                return True
            else:
                return False

        # Next we try different vertices as the next step
        # But we shouldn't try for 0 since we already did so by including 0 as the starting point in in the hamcycle
        for v in range(1, self.totalV):
            # We can set a variable to check for adjaceny vertex of the previously added vertex using Boolean
            check = False
            ...
            if(v == previousVertex):
                check = True
            # Check if current vertex already in path
            for i in range(0, totalvisited):
                if Hamiltonian.vertices[i] == v:
                    check = True
                    break
            if not check:
                Hamiltonian.vertices[totalvisited] = v
                if self.tryVisiting(v, totalvisited + 1, Hamiltonian) == True:
                    return True
                # Remove current vertex if it doesn't
                # lead to a solution
                Hamiltonian.vertices[totalvisited] = -1
